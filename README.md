...
This script, `accuracy.py`, is designed to visualize the performance metrics of a trained cell counting model. It reads evaluation results from a CSV file and generates bar plots for accuracy and R² scores on the training, validation, and test datasets. The visualizations help in understanding the model's performance and identifying areas for improvement.


## Features

- **Visualization of Accuracy**: Plots the accuracy of the model within ±5% of the actual cell count for training, validation, and test sets.
- **Visualization of R² Scores**: Plots the R² scores for the model on the same datasets.
- **Saves Plots**: Automatically saves the generated plots as PNG images in the specified directory.
- **Easy Integration**: Simple script that can be integrated into the model evaluation pipeline.
...
├── models/
│   ├── accuracy_plot.png       # Will be created by the script
│   ├── r2_plot.png             # Will be created by the script
├── accuracy.py
└── ...
```

- **features/**: Contains the `evaluation_results.csv` file generated by the model evaluation script (`model_run.py`).
- **models/**: Directory where the generated plots will be saved.
- **accuracy.py**: The script to generate accuracy and R² score plots.

## Understanding the Output

After running the script, you will find two PNG images in the `models/` directory:

1. **accuracy_plot.png**: A bar plot displaying the accuracy (in percentage) of the model within ±5% of the actual cell count for the training, validation, and test sets.

   - **X-axis**: Dataset names (Training, Validation, Test).
   - **Y-axis**: Accuracy percentage.
   - **Bar Labels**: Exact accuracy values displayed above each bar.

2. **r2_plot.png**: A bar plot displaying the R² scores for the model on the training, validation, and test sets.

   - **X-axis**: Dataset names.
   - **Y-axis**: R² score ranging from -1 to 1.
   - **Bar Labels**: Exact R² values displayed above each bar.

These plots provide a visual representation of how well the model is performing on different datasets, making it easier to assess its generalization capabilities and identify any overfitting or underfitting issues.
